...
network, so the listing is never marked exported.

Left out on purpose, and reported as warnings:

- Images. Uploaded photos are private links. A buyer-facing copy needs its own approval
...
def approval_problems(listing: Any, taxonomy: dict | None = None) -> list[str]:
    ...
    pending = sorted(c.claim for c in listing.claims if c.asserted_by_artisan and not c.coordinator_verified)
    if pending:
...
def build_export(listing: Any, taxonomy: dict | None = None) -> ExportBuild:
    ...
    warnings = [
        "No images are included: uploaded photos are private, and there is no approved buyer-facing copy yet.",
    ]
